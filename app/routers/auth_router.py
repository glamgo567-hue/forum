...
async def registration(user_data: UserRegister, db: AsyncSession = Depends(get_db)):
    query = (await db.execute(select(User).where(or_(User.username==user_data.username, User.email==user_data.email)))).scalar_one_or_none()
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    query = (await db.execute(select(User).where(User.username==form_data.username))).scalar_one_or_none()
    if query is None:
        raise HTTPException(status_code=401, detail="Incorrect username or password")
    verify = verify_password(form_data.password, query.hashed_password)
    if not verify:
        raise HTTPException(status_code=401, detail="Incorrect username or password")
    token = create_access_token({"sub": str(query.id)})
